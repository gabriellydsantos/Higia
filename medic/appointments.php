            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($row["status"] == "Concluído") {
                        echo '<td><button class="custom-badge status-green" onclick="updateStatus(' . $row["id"] . ')">' . $row["status"] . '</button></td>';
                    } else {
                        echo '<td><button class="custom-badge status-red" onclick="updateStatus(' . $row["id"] . ')">' . $row["status"] . '</button></td>';
                    }
